                        ...
                        @if($r->parentid == 0)
                          <tr data-widget="expandable-table" aria-expanded="false">
                            <td>
                              ...
                              {{ $r->name }} 
                              <button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#modal-edit{{$r->menuid}}"><i class="fas fa-edit"></i></button>
                              <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#modal-delete" onclick="setDeleteUrl('{{url('/sysadmin/menumanager/delete')}}/{{ $r->menuid }}')"><i class="far fa-trash-alt"></i></button>
                            </td>
                          </tr>
                                    ...
                                    @foreach($menulist as $child)
                                      @if($child->parentid == $r->menuid)
                                        <tr data-widget="expandable-table" aria-expanded="false">
                                          <td>
                                            {{ $child->name }} 
                                            <button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#modal-edit{{$child->menuid}}"><i class="fas fa-edit"></i></button>
                                            <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#modal-delete" onclick="setDeleteUrl('{{url('/sysadmin/menumanager/delete')}}/{{ $r->menuid }}')"><i class="far fa-trash-alt"></i></button>
                                          </td>
                                        </tr>
                                      @endif
                    ...
                    <div class="form-group">
                      ...
                      <option value="">None</option>
                        @foreach($parentlistedit as $p)
                          <option value="{{ $p->menuid }}"
                            @if($p->menuid == $r->parentid) selected @endif>
                            {{ $p->menuid }} - {{ $p->name }}
                          </option>
                        @endforeach
                      </select>
